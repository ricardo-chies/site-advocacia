...
import { Menu, X, Lock } from "lucide-react"; // Removido ArrowLeft daqui também se não for usar em outro lugar
    ...
    >
      <div className="container">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          {/* LADO ESQUERDO: Apenas Logo (Botão Voltar Removido) */}
          ...
          <div className="hidden lg:flex items-center gap-1">
             {navLinks.map((link) => (
              <a
                ...
                onMouseEnter={(e) => { (e.target as HTMLElement).style.color = "oklch(74% 0.12 80)"; }}
                onMouseLeave={(e) => { (e.target as HTMLElement).style.color = "oklch(85% 0.02 245)"; }}
              >
                {link.label}
              </a>
            ))}
        ...
        {isOpen && (
          ...
          >
            {/* Botão Voltar Mobile REMOVIDO daqui também */}
            ...
            <div className="px-4 pt-2 space-y-2">
               <Link
                href="/admin"
                className="block w-full text-center px-5 py-3 rounded-lg text-sm font-semibold transition-colors border"
                style={{ borderColor: "oklch(74% 0.12 80 / 0.3)", color: "oklch(74% 0.12 80)" }}
              >
                Painel de Admin
              </Link>
              <a
